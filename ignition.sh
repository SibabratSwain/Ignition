...
# Optimized for use with version 3.5.42-st of the Wrapper.
...
APP_NAME="Ignition-Gateway"
APP_LONG_NAME="Ignition-Gateway"
...
# Wrapper
WRAPPER_CMD="./ignition-gateway"
WRAPPER_CONF="./data/ignition.conf"
...
# Source this file before all others to allow var overrides
# NO_SOURCE_IA_UTIL is set in ignition-util.sh to allow callbacks to this script when functions are overridden
IA_UTIL_FILE="$REALDIR/ignition-util.sh"
if [ -f "$IA_UTIL_FILE" ] && [ -z "$NO_SOURCE_IA_UTIL" ]; then
    if [ ! -x "$IA_UTIL_FILE" ] ; then
        # We should stop here because the configuration expected in the utilities file will not be loaded.
        eval echo `gettext 'Found $IA_UTIL_FILE but could not execute it. Please make sure that the file has execute permissions.'`
        exit 1
    fi
    # shellcheck source=ignition-util.sh
    IGNITION_COMMAND="$1" . "$IA_UTIL_FILE" # We want this to just get passed to our utilities file.
fi
