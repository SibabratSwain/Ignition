...
def getsitepackages():
    ...
    # Ignition modification from standard Jython 2.7.1 sites.py
    # Go from sys.path in Ignition, since PREFIXES just returns the
    # jython-2.7.1-ia.jar cache directory, not the pylib directory
    for prefix in sys.path:
        # Skip the __classpath__ and __pyclasspath__/ paths
        if prefix[:2] == "__":
            continue
        ...
        # Since this is now Ignition-specific code, we know we're in 
        # Jython, so we're just including the Jython code branch for this.
